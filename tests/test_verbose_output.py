...
def test_verbose_math(equation: str, expected_output: str) -> None:
    """Test that the individual operators are outputting correctly."""
    output = roll(equation, verbose=True, roll_option=RollOption.Minimum)

    if str(output) != expected_output:
        raise Exception(
            f"The string output of the roll was not what we expected."
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "4d6K3",
            "\n".join(
                ["Rolled: 4d6: [1, 1, 1, 1]", "Keeping highest: 3: [1, 1, 1]", "3"]
            ),
        ),
        (
            "10d50k7",
...
def test_verbose_dice(equation: str, expected_output: str) -> None:
    """Test that the output with the verbose option matches our expectations."""
    output = roll(equation, verbose=True, roll_option=RollOption.Minimum)
